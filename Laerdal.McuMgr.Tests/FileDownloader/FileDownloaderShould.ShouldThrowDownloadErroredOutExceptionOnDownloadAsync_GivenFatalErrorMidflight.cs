...
namespace Laerdal.McuMgr.Tests.FileDownloader
{
    ...
    public partial class FileDownloaderShould
    {
        ...
        public async Task ShouldThrowDownloadErroredOutExceptionOnDownloadAsync_GivenFatalErrorMidflight(string testcaseDescription, int maxRetriesCount)
        {
            // Arrange
            var mockedFileData = new byte[] { 1, 2, 3 };
            const string remoteFilePath = "/path/to/file.bin";

            var mockedNativeFileDownloaderProxy = new MockedGreenNativeFileDownloaderProxySpy4(new McuMgr.FileDownloader.FileDownloader.GenericNativeFileDownloaderCallbacksProxy(), mockedFileData);
